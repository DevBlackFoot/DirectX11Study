...
namespace GameEngineSpace
{
	...
	void Transform::UpdateLocalTM()
	{
		...
		localTM = Matrix::CreateScale(localScale) * Matrix::CreateRotationZ(localRotation.z) *
			Matrix::CreateRotationX(localRotation.x) * Matrix::CreateRotationY(localRotation.y) * Matrix::CreateTranslation(localPosition);
	...
	void Transform::DecomposeMatrix(Vector3& scale, Vector3& rotation, Vector3& position, Matrix matrix)
	{
		...
		scale.x = sqrtf(powf(matrix.m[0][0], 2.0f) + powf(matrix.m[1][0], 2.0f) + powf(matrix.m[2][0], 2.0f));
		scale.y = sqrtf(powf(matrix.m[0][1], 2.0f) + powf(matrix.m[1][1], 2.0f) + powf(matrix.m[2][1], 2.0f));
		scale.z = sqrtf(powf(matrix.m[0][2], 2.0f) + powf(matrix.m[1][2], 2.0f) + powf(matrix.m[2][2], 2.0f));
