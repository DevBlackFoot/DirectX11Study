...
{
	void Texture::Initialize(const std::string& _textureName, const std::wstring& path)
	{
		...
		HRESULT hr = ::CreateTexture(DX11GraphicsCore::GetInstance()->GetDevice(), image.GetImages(), image.GetImageCount(), image.GetMetadata(),
