...
namespace GraphicsEngineSpace
{
	...
	bool RenderTargetTexture::Init(ID3D11Device* device, int width, int height, UINT MSAAQuality)
	{
		...
		// 텍스쳐 생성
		hr = device->CreateTexture2D(&textureDesc, 0, &renderTexture);
		...
		hr = device->CreateRenderTargetView(renderTexture, &rtvDesc, &renderTarget);
		if (FAILED(hr) == true)
		{
		...
		hr = device->CreateShaderResourceView(renderTexture, &srvDesc, &shaderResource);
		if(FAILED(hr) == true)
		{
