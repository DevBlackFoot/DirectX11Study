...
/// </summary>
///
///	교수님의 재귀 형태의 ASE파서를 Normal Map까지 개조한 것을 들고온 파서 클래스
///
...
public:
	...
	bool Convert_Optimize(GeomObject* pGeomObj);		// 버텍스(노말), 텍스쳐 값으로 최적화를 해 준다.
	bool Convert_OptimizeLight(GeomObject* pGeomObj);	// 텍스쳐 없이 노말값으로만 최적화.
...
public:
	...
	/// 한 씬에 N개의 기하 오브젝트가 존재할 수 있다.
	GeomObject* m_TempGeomObject;				// 기하 오브젝트의 데이터가 들어갈 구조체(임시)
	vector<GeomObject*> m_GeomObjList;				// 지오메트리들
...
private:
	...
	Vector3 Parsing_NumberVector3Mat();	// 3개의 Float를 순서 상관 없이 읽어서 벡터로 리턴한다.
